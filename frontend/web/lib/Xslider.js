	...
	$.fn.Xslider=function(settings){
		...
		this.each(function(){
			var scrollobj=settings.scrollobj ? $(this).find(settings.scrollobj) : $(this).find("ul"),
			    ...
			    btnright=$(this).find("a.next"),
			    btnleft=$(this).find("a.prev");
			...
			btnleft.click(function(){
				...
				if(!moving){
					...
					if(settings.loop=="cycle"){
						...
						$.fn.Xslider.sn.animate(scrollobj,0,settings.dir,settings.speed,function(){moving=false;scrollobj.css(pos,"0px");});
					}else{
                        ...
                        var left=$(watchListEl).find("ul").position().left;
                        if(left>=(-$(window).width()/2*4)){
			                $(watchTypeEl).find("img").eq(0).addClass("active");
			                $(watchTypeEl).find("img").eq(0).siblings().removeClass("active");
		                }else if(left<(-$(window).width()/2*4) && left>=(-$(window).width()/2*6)){
			            $(watchTypeEl).find("img").eq(1).addClass("active");
			            $(watchTypeEl).find("img").eq(1).siblings().removeClass("active");
		                }else{
			                $(watchTypeEl).find("img").eq(2).addClass("active");
			                $(watchTypeEl).find("img").eq(2).siblings().removeClass("active");
		                }
						btnright.removeClass("agrayright");
					}
				}
				...
				if(!moving){
					...
					if(settings.loop=="cycle"){
						...
						if(offsetnow<offset-(unitlen*unitdisplayed-viewedSize)){
							...
							$.fn.Xslider.sn.animate(scrollobj,-(offset-(unitlen*unitdisplayed-viewedSize)),settings.dir,settings.speed,function(){moving=false;});//滚动到最后一个位置;
							offsetnow=offset-(unitlen*unitdisplayed-viewedSize);
                        ...
                        var watchTypeEl = watchListBoxEl + ' .watch-type';
                        var left=$(watchListEl).find("ul").position().left;
                        if(left>=(-$(window).width()/2*2)){
			                $(watchTypeEl).find("img").eq(0).addClass("active");
			                $(watchTypeEl).find("img").eq(0).siblings().removeClass("active");
                        }else if(left<(-$(window).width()/2*2) && left>=(-$(window).width()/2*4)){
                            $(watchTypeEl).find("img").eq(1).addClass("active");
                            $(watchTypeEl).find("img").eq(1).siblings().removeClass("active");
                        }else{
                            $(watchTypeEl).find("img").eq(2).addClass("active");
